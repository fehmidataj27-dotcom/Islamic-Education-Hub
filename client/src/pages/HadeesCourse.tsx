...
import hadeesHero from "file:///C:/Users/Fehmida%20Taj/.gemini/antigravity/brain/4ba9dfa2-40dd-4da3-86ca-15ecb75f168c/hadees_course_banner_1772547499657.png";
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
export default function HadeesCourse() {
    const { lang } = useTheme();
    const [activeTab, setActiveTab] = useState("foundations");
    const [hadeesVideos, setHadeesVideos] = useState<any[]>([]);
    ...
    const { data: announcements } = useCategoryAnnouncements("Hadees");


    useEffect(() => {
        // Award Hadees badge
        checkAchievements({ visitedHadees: true });
    }, []);

    useEffect(() => {
        if (videosData) {
            // Support both Hadees and Hadith categories
            setHadeesVideos(videosData.filter((v: any) => v.category === 'Hadees' || v.category === 'Hadith'));
        }
    }, [videosData]);

    const t = {
        title: lang === 'en' ? 'Hadees Course' : 'حدیثِ مبارکہ کورس',
        subtitle: lang === 'en' ? 'Preserving and understanding the prophetic traditions' : 'فرامینِ رسول اللہ ﷺ کی حفاظت اور ان کی تفہیم',
        ...
        deleteDesc: lang === 'en' ? "This will permanently remove this Hadees lecture." : "یہ مستقل طور پر اس حدیث کے لیکچر کو ہٹا دے گا۔",
    ...
    return (
        <CourseGuard courseName="Hadees">
            <div className="space-y-8 pb-20">
                <CourseHero
                    title={t.title}
                    subtitle={t.subtitle}
                    badgeText={lang === 'en' ? 'Wisdom of the Prophet ﷺ' : 'حکمتِ نبوی ﷺ'}
                    color="violet"
                    illustration={hadeesHero}
                >
                    <div className="hidden md:block">
                        <div className="w-24 h-24 rounded-full bg-violet-500/10 flex items-center justify-center border border-violet-500/20 backdrop-blur-sm animate-pulse">
                            <PlayCircle className="h-10 w-10 text-violet-600" />
                        </div>
                    </div>
                </CourseHero>
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <Tabs value={activeTab} defaultValue="foundations" className="w-full" onValueChange={setActiveTab}>
                            <div className="flex justify-start mb-8 overflow-x-auto pb-2 scrollbar-hide">
                                <TabsList className="bg-muted/50 p-1 h-14 rounded-2xl border border-border/50 flex w-max">
                                    <TabsTrigger value="foundations" className="rounded-xl px-6 data-[state=active]:bg-violet-600 data-[state=active]:text-white transition-all duration-300">
                                        <BookOpen className="h-4 w-4 mr-2" /> {t.foundations}
                                    </TabsTrigger>
                                    <TabsTrigger value="sessions" className="rounded-xl px-6 data-[state=active]:bg-violet-600 data-[state=active]:text-white transition-all duration-300">
                                        <Video className="h-4 w-4 mr-2" /> {t.sessions}
                                    </TabsTrigger>
                                    <TabsTrigger value="notes" className="rounded-xl px-6 data-[state=active]:bg-violet-600 data-[state=active]:text-white transition-all duration-300">
                                        <BookOpen className="h-4 w-4 mr-2" /> {lang === 'en' ? 'Course Notes' : 'کورس نوٹس'}
                                    </TabsTrigger>
                                    <TabsTrigger value="tests" className="rounded-xl px-6 data-[state=active]:bg-violet-600 data-[state=active]:text-white transition-all duration-300">
                                        <ClipboardList className="h-4 w-4 mr-2" /> {lang === 'en' ? 'Take a Test' : 'ٹیسٹ لیں'}
                                    </TabsTrigger>
                                    <TabsTrigger value="groups" className="rounded-xl px-6 data-[state=active]:bg-violet-600 data-[state=active]:text-white transition-all duration-300">
                                        <Users className="h-4 w-4 mr-2" /> {lang === 'en' ? 'Course Groups' : 'گروپس'}
                                    </TabsTrigger>
                                    <TabsTrigger value="updates" className="rounded-xl px-6 data-[state=active]:bg-violet-600 data-[state=active]:text-white transition-all duration-300">
                            ...
                            <TabsContent value="foundations" className="mt-0">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    {[
                                        { title: lang === 'en' ? 'Terminology' : 'اصطلاحات حدیث', desc: lang === 'en' ? 'Key concepts in Hadees verification.' : 'حدیث کی تصدیق کے لیے بنیادی اصطلاحات۔' },
                                        { title: lang === 'en' ? 'Authenticity' : 'صحتِ حدیث', desc: lang === 'en' ? 'Levels of reliability: Sahih, Hasan, Daif.' : 'حدیث کے درجات: صحیح، حسن اور ضعیف۔' },
                                        { title: lang === 'en' ? 'Compilers' : 'ائمہ حدیث', desc: lang === 'en' ? 'Lives of Imam Bukhari, Muslim, and others.' : 'امام بخاری، امام مسلم اور دیگر ائمہ کی سیرت۔' },
                                    ].map((item, idx) => (
                                        <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}>
                                            <Card className="h-full border-violet-500/10 hover:shadow-lg transition-transform hover:-translate-y-1">
                                                <CardHeader>
                            ...
                            <TabsContent value="sessions" className="mt-0">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    {(user?.role?.toLowerCase() === 'admin' || user?.role?.toLowerCase() === 'teacher') && <MediaUpload category="Hadees" />}
                                </div>

                                {hadeesVideos.length === 0 ? (
                                    <Card className="p-12 text-center border-dashed">
                                        <div className="flex flex-col items-center gap-4 text-muted-foreground">
                                            <Video className="h-12 w-12 opacity-20" />
                                            <p>{lang === 'en' ? 'No Hadees sessions available yet.' : 'ابھی تک کوئی حدیث مبارکہ سیشن دستیاب نہیں ہے۔'}</p>
                                        </div>
                                    </Card>
                                ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {hadeesVideos.map((video) => (
                                            <Card key={video.id} className="group overflow-hidden hover:shadow-2xl transition-all duration-500 border-border/40 hover:border-violet-500/30 flex flex-col h-full bg-card/40 backdrop-blur-sm rounded-2xl">
                                                ...
                                                <CardFooter className="p-5 pt-0 flex gap-2">
                                                    <Button className="flex-1 rounded-xl h-11 gap-2 font-bold shadow-lg bg-violet-600 hover:bg-violet-700" onClick={() => setPlayingVideo(video)}>
                            ...
                            <TabsContent value="notes" className="mt-0">
                                <CourseNotesSection courseId="hadees" />
                            </TabsContent>

                            <TabsContent value="tests" className="mt-0">
                                <CourseTestSection courseId="hadees" />
                            </TabsContent>

                            <TabsContent value="groups" className="mt-0">
                                <h2 className="text-2xl font-bold mb-6">{lang === 'en' ? 'Hadees Groups' : 'حدیث گروپس'}</h2>
                                <CourseGroupsSection category="Hadees" />
                            </TabsContent>
                                        ...
                                        announcements.map((ann: any, idx: number) => (
                                            <motion.div key={ann.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }}>
                                                <Card className="overflow-hidden border-none shadow-lg bg-white rounded-[2rem]">
                                                    <div className="bg-gradient-to-r from-violet-600 to-indigo-600 p-6 flex items-center justify-between text-white">
                                                        ...
                                                        {ann.fileType === 'voice' && ann.fileUrl && (
                                                            <div className="p-4 rounded-2xl bg-muted/30 border border-border/50 max-w-md">
                                                                <div className="flex items-center gap-2 mb-3">
                                                                    ...
                                                                    <span className="text-[10px] font-black uppercase tracking-widest text-violet-600/60">Voice Message</span>
                                                                </div>
                    ...
                    <aside className="lg:w-[30%] xl:w-[25%] order-1 lg:order-2">
                        <div className="sticky top-24 space-y-6">
                            <Card className="border-none shadow-xl rounded-[2.5rem] bg-violet-950 text-white overflow-hidden relative group">
                                ...
                                <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-violet-950 to-transparent" />
                                <CardHeader className="p-8 relative z-10">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-violet-200/60 text-[9px] font-black uppercase tracking-[0.3em] mt-1">Prophetic Wisdom</p>
                                        </div>
                                    </div>
                                </CardHeader>
                                <CardContent className="p-8 pt-0 grid grid-cols-1 gap-4 relative z-10">
                                    <Button
                                        variant="outline"
                                        className="h-16 justify-start gap-4 rounded-2xl border-white/10 bg-white/5 hover:bg-white/10 hover:border-violet-400/50 text-white px-6 transition-all"
                            ...
                            <Card className="border-none shadow-xl rounded-[2.5rem] bg-white p-8 relative overflow-hidden group">
                                <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:scale-110 transition-transform">
                                    <Info className="h-24 w-24 text-violet-900" />
                                </div>
                                <div className="relative z-10 space-y-4">
                                    <h3 className="text-xl font-black text-violet-950 leading-tight">Learn Authentic Hadith</h3>
                                    <p className="text-muted-foreground text-sm font-bold leading-relaxed">
                                        Study the Sahih Sittah and gain deep insight into the Sunnah of Prophet Muhammad ﷺ.
                                    </p>
                                    <Badge className="bg-violet-100 text-violet-600 border-none px-4 py-1.5 rounded-full font-black text-[9px] uppercase tracking-widest">
                                        Verified Content
                                    </Badge>
                                </div>
                            </Card>
